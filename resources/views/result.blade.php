...
<head>
    <title>Rosario Costantini | HOME</title>
          ...
          <div class="row no-gutters">
            <div class="col-lg-3 sidebar d-none d-lg-block">
              <div class="sidebar">
                <div class="container">
                  <!-- ================================== FILTROS ========================================== -->
                  <form class="side-form" action="{{url('/')}}/result/0" method="get">
                    <div class="form-group form-custom">
                      <label class="pl-1 pb-1 select-label" for="">OPERACIÓN</label>
                      <select name="operacion" class="form-control form-sm">
                        <option value="" selected>Tipo de Operación</option>
                        @foreach($operaciones as $operacion)
                        <option value="{{$operacion}}"  @if(isset($_GET['operacion']) && $_GET['operacion']==$operacion)) selected @endif>{{$operacion}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group form-custom">
                      <label class="pl-1 pb-1 select-label" for="">PROPIEDAD</label>
                      <select name="tipo" class="form-control form-sm">
                        <option value="" selected>Tipo de Propiedad</option>
                        @foreach($tiposDePropiedad as $tipo)
                        <option value="{{$tipo}}"  @if(isset($_GET['tipo']) && $_GET['tipo']==$tipo)) selected @endif >{{$tipo}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group form-custom">
                    <label class="pl-1 pb-1 select-label" for="">Precio</label>
                      ...
                      <select name="moneda" class="form-control mon-sel form-sm">
                        <option value="">Mon.</option>

                        <option value="U$S">US$</option>
                        <option value="$">AR$</option>
                      </select>
                      <input name="valorMinimo" type="" class="form-control mon-inp form-sm" placeholder="Desde" value="@if(isset($_GET['valorMinimo'])){{$_GET['valorMinimo']}}@endif">
                      <input name="valorMaximo" type="" class="form-control mon-inp form-sm" placeholder="Hasta" value="@if(isset($_GET['valorMaximo'])){{$_GET['valorMaximo']}}@endif">
                      </div>
                    </div>
                    <div class="form-group form-custom">
                      <label class="pl-1 pb-1 select-label" for="">ZONA</label>
                      <select name="zona" class="form-control form-sm">
                        <option value="" selected>Zonas</option>
                        @foreach($zonas as $miCabeZona)
                        <option value="{{$miCabeZona}}" @if(isset($_GET['zona']) && $_GET['zona']==$miCabeZona)) selected @endif >{{$miCabeZona}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group form-custom">
                      <label class="pl-1 pb-1 select-label" for="">BARRIO</label>
                      <select name="barrio" class="form-control form-sm">
                        <option value="" selected>Localidades y Barrios</option>
                        @foreach($barrios as $barrio)
                        <option value="{{$barrio}}"  @if(isset($_GET['barrio']) && $_GET['barrio']==$barrio)) selected @endif>{{$barrio}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group form-custom">
                      ...
                      <select class="form-control form-sm">
                        <option value="any_Purpose" selected>Tipo de Lote</option>
                    ...
                    <div class="form-group form-custom">
                      ...
                      <select name="habitaciones" class="form-control form-sm">
            ...
            <div class="col-lg-9 bg-secondary catalog-content">
              <div class="search-result -list active" data-list="list">
                <nav aria-label="breadcrumb">
              <ol class="breadcrumb mt-4 pb-0">
                @if(isset($_GET))
                  @foreach($_GET as $filtro)
                    @if($filtro!="")
                      <li class="breadcrumb-item"><a href="index-2.html">{{$filtro}}</a></li>
                      @endif
                  @endforeach
                @endif
              </ol>
                ...
                <!-- =====================  PROPIEDADES =============================
                ================================================================ -->
                <div class=" mb-3 res-title">
                <h4>
                  @if(!isset($errores['propiedades']))
                  Tenemos {{$cantidadDePropiedades}} @php if($cantidadDePropiedades > 1){echo 'propiedades';} else{echo 'propiedad';}; @endphp
                  @endif
                </h4>
                </div>
                <div class="row">
                  ...
                  <!-- ============================= PROPIEDAD ========================== -->
                  @if(!isset($errores['propiedades']))
                  @php $GLOBALS['indice'] = -1; @endphp
                  @foreach($propiedadesFiltradas as $propiedad)
                  <div class="col-xl-6 col-lg-6 col-md-6 d-flex">
                    @php $GLOBALS['indice']++ @endphp
                    <div class="flat-card">
                      <a href="propiedad/{{$propiedad['in_cod']}}">
                      <div class="flat-card_top">
                        <div class="tag -sale">{{$propiedad['operacion']}}</div>
                          @if($propiedad['in_lot']!="")
                          <div class="tag -sale type">{{$propiedad['in_lot']}}</div>
                          @endif
                          <div class="-resp-bg-img" style="background-image: url({{$propiedad['img_princ']}});"></div></a>
                      </div>
                      <a>
                      <div class="flat-card_bot">
                        <h5 class="flat-card_title"><a href="propiedad">{{$propiedad['titulo']}}</a></h5>
            ...
            <ul class="pagination" style="display:flex; flex-direction:row; justify-content:center; margin-bottom:10px;">
              <li class="page-item"><a class="page-link" href="#"><<</a></li>
              <li class="page-item"><a class="page-link" href="0?{{http_build_query($_GET)}}">1</a></li>
              <li class="page-item"><a class="page-link" href="1?{{http_build_query($_GET)}}">2</a></li>
              <li class="page-item"><a class="page-link" href="2?{{http_build_query($_GET)}}">3</a></li>
              <li class="page-item"><a class="page-link" href="3?{{http_build_query($_GET)}}">4</a></li>
              <li class="page-item"><a class="page-link" href="4?{{http_build_query($_GET)}}">5</a></li>
              <li class="page-item"><a class="page-link" href="#">>></a></li>
            </ul>
              <!-- Scroll top button-->
      ...
      <div class="mob-filter col-lg-12">
        <div class="container">
          <div class="text-right close-nav"><i class="fas fa-times"></i></div>
              <!-- ================================== FILTROS ========================================== -->
              <form class="side-form" action="result">
